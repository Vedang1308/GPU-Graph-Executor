...
class MatMulOp(Op):
    ...
    def infer_shape(self, node, input_shapes):
        ...
        dim_A_0 = shape_A[1] if node.matmul_attr_trans_A else shape_A[0]
        dim_A_1 = shape_A[0] if node.matmul_attr_trans_A else shape_A[1]
        dim_B_0 = shape_B[1] if node.matmul_attr_trans_B else shape_B[0]
        dim_B_1 = shape_B[0] if node.matmul_attr_trans_B else shape_B[1]
        assert dim_A_1 == dim_B_0
        return (dim_A_0, dim_B_1)
    ...
    def memory_plan(self, feed_shapes):
        ...
        for node in self.topo_order:
            if node not in feed_shapes:
                ...
                if node not in self.node_to_arr_map or self.node_to_arr_map[node].shape != shape:
